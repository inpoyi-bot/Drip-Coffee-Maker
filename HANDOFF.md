...
> **进度(6-29 更新)**:
> - ✅ **A 已全清**(换号新 key、一跑三用):CoT 泄露修复**已验证 + 合 main**;**demo 5/5 干净逐字稿已替换 `docs/demo-arc.md`**;S2 e2e + 跨会话持久化通过。
> - ✅ **B 已全清**:7 个 eval(E1a/E2a/E4/E6/E7/E8/E9)跑完;第一轮 E2a/E4 红、E9 黄,经一轮校准(涩/通道优先级闸门、客观交叉验证硬闸门、认峰回退、终止收尾话、禁用口味层枚举)**全部转绿并验证、已合 main**。**S1 基线全绿。**
> - ⛔ **C(S3)E11a/E11b**——阻塞于口味层规则(S3 对话),定稿写进 instruction 再测——见 §9。
> **下一步:只剩 C,等 S3 口味层规则定稿**(见另一个对话)。

---

## 跑测排序(省额度,务必先读)
日配额 **20** + **RPM=5**(每分钟5次),带工具每轮~2次调用 → 间隔 **15s**。免费层一天清不完 A+B+C。

**A. 一条 5 杯弧线 = 一跑三用(~10次,先跑这件)** —— 跑在 `feat/no-cot-leak` 分支(含 CoT 修复)、用 `DatabaseSessionService`(`sqlite+aiosqlite://`):
- ① **CoT 泄露验证**(盯第2杯)→ 干净就把分支合 main;
- ② **demo 5/5 逐字稿** → 干净就替换 `docs/demo-arc.md`(= §1–§5 那件事);
- ③ **S2 e2e** → start_bag/record_cup 被调 + 跨会话持久化(= §8 那件事)。
- **别把这三件分开跑、白烧额度。** 空响应判"未测"别信假绿。

**B. 7 个 S1 eval(§7,~10–14次)** —— A 之后用剩余额度尽量跑,溢出的留后天。

**C. E11a/E11b(§9,~4–8次)** —— **阻塞**:要先有 S3 口味层规则(另一个对话)。规则没落地别跑(必 fail)。

> **预算**:A(~10)必清;A+B ≈ 20–24 已顶到/略超日配额 20,B 大概率只跑大半;C 看 S3。**想 A+B+C 一天全清 → 绑卡(Tier 1)**,demo 日前迟早要做。
