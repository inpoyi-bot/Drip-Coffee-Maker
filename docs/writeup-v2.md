...
**State-write guardrails**: `record_cup` enforces invariants before writing — a terminate decision cannot co-occur with an adjust direction; contradictory records are rejected with an error rather than stored. This is a data-contract guardrail, not adversarial-input defense, and it earned its keep in production during this build (see the EDD section): when instruction-following degraded, the invariant rejected a contradictory record at the write boundary and kept the trajectory store clean. Credentials live in `.env`; the session database is git-ignored.
...
**Deployability**: the claim is precise — a locally reproducible ADK run path with documented dependencies, environment setup, launch commands, and SQLite-backed session persistence verified across restarts. No cloud deployment, Docker, or CI/CD is claimed.
