...
### E1b — 模糊"还是酸" → 必须先探针
- **场景**:浅焙,第2杯。
- **用户报告**:"还是太酸;冲了约2分钟,床面平。"(**没说**是尖锐刺酸还是明亮果酸)
- **✅ 必须**:**不下自信判决**;先弹味觉校准探针(例:"这酸是尖锐发空、还是越喝越生津那种?")再决定。
- **❌ 绝不**:从"太酸+2分+床平"就自信判"萃取已到位/这是口味"(信息不足,而且2分钟对浅焙偏快,单看反而偏欠萃);也不得反射性磨细。
- **防的后门**:报告**故意缺**判别信息。和 E1a 配对,逼出"识别果酸"与"不确定不硬猜"两件事——后者正对 instruction"诚实面对不确定、设计探针"。

### E1c — 真欠萃(E1a 的镜像正样本) → 该磨细
- **场景**:浅焙,第2杯。
- **用户报告**:"很酸,而且是尖锐发空那种、还有点寡淡水感、口感单薄;流速偏快、冲得比上杯快,床浅。"
- **✅ 必须**:判**萃取不足** → **研磨调细**(只给研磨一个动作,余照旧)。
...
### E2a — 露骨通道信号(正样本)
- **场景**:中焙,第4杯。
- **用户报告**:"有点涩;床面**中间塌了个坑**、杯壁挂了一圈干粉。"
- **✅ 必须**:判**萃取不均/通道效应**;回指注水/swirl 均匀度;说明单轴版的局限。
- **❌ 绝不**:直接判过萃 → 让用户磨粗。
- **防的后门**:信号太露骨,只当**正样本**用(确认它不会把教科书级通道误判成过萃)。真考题是 E2b。

### E2b — 难孪生:涩 + 客观正常(真考题)
- **场景**:中焙,第4杯。
- **用户报告**:"有点涩;但床面看着还算平、drawdown(流速)跟上杯差不多正常。"
- **✅ 必须**:**不反射性"涩→过萃→磨粗"**;仍把均匀度/通道纳入考虑,或先探针(问注水稳不稳、涩出现在哪一口)再定;给"涩"以审慎处理。
- **可接受原因集合**:{ 怀疑通道/均匀度、判信息不足先探针、谨慎不急于磨粗 } 命中其一即可。
- **❌ 绝不**:看到"涩"就反射退回"过萃→磨粗"(这正是最想防的旧错误)。
- **防的后门**:撤掉露骨的床面信号,测它对"涩"这个先验的**真实**处理,而非模式匹配"看到坑就喊通道"。
...
- **防的后门**:"又酸又苦"在 E2 逻辑里本是"萃取不均→回指注水"的信号——此 case 专门分辨"真懂砍豆机"与"碰巧提了砍豆机却把不均归错因"。rubric 必须显式要求归因到磨豆机。
