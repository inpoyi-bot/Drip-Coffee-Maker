    ...
    <div className="min-h-[100dvh] w-full bg-background flex flex-col items-center">
      <header className="w-full max-w-md border-b border-border bg-background px-4 py-3 flex items-center justify-between sticky top-0 z-10">
        ...
        <nav className="flex gap-4 text-sm font-medium">
          <Link href="/" className={matchHome ? 'text-foreground font-bold' : 'text-muted-foreground'}>Start</Link>
          <Link href="/feedback" className={matchFeedback || matchDiagnosis ? 'text-foreground font-bold' : 'text-muted-foreground'}>Feedback</Link>
          <Link href="/trajectory" className={matchTrajectory ? 'text-foreground font-bold' : 'text-muted-foreground'}>Trajectory</Link>
        </nav>
      </header>
